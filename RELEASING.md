...
/tmp/httk-web-test/bin/python -c "import httk.web"
```

Replace `0.1.0` with the version being tested. Unlike `httk-core`, `httk-web`
has runtime dependencies (including `httk-core`, `starlette`, and `jinja2`), so
`--no-deps` is not appropriate here: `import httk.web` pulls in `starlette` at
import time. The `--extra-index-url` lets pip resolve those dependencies (which
are published to the real PyPI) while the package under test comes from TestPyPI.
