...
from httk.serve.web.providers import ProviderContext, TableColumn, TablePage, TableRequest, _validate_site_route
...
TABLE_ENDPOINT = "/_httk/serve/table/page"
TABLE_ASSET_ROOT = "/_httk/serve/assets"
...
class TableTokenSigner:
    ...
    def __init__(self, secret: str | bytes | None = None) -> None:
        if secret is None:
            secret = os.environ.get("HTTK_SERVE_WEB_TABLE_TOKEN_SECRET")
        if secret is None:
...
class TableRuntime:
    ...
    def _provide(
        ...
        *,
        ...
        try:
            ...
            _LOGGER.exception("httk.serve.table provider %r failed for route %r", provider, route)
        ...
        for row in page.rows:
            if row_template is not None:
                ...
                for column in page.columns:
                    classes = ["httk-serve-table__cell"]
                    if column.align is not None:
                        classes.append(f"httk-serve-table__cell--{column.align}")
    ...
    def _render_table_shell(
        ...
        *,
        ...
        headers = "".join(
            (
                f'<th scope="col" class="httk-serve-table__header{_column_modifier(column)}">'
        ...
        if live:
            assets = (
                f'<link rel="stylesheet" href="{internal_root}/assets/serve-table.css">'
                f'<script defer src="{internal_root}/assets/serve-table.js"></script>'
            )
        return (
            f"{assets}<section class=\"httk-serve-table\" data-httk-serve-table=\"1\" data-route=\"{route}\" "
            ...
            "<nav class=\"httk-serve-table__pager\" aria-label=\"Table pagination\">"
            f'<button type="button" data-httk-serve-table-previous data-token="{previous_attr}"{previous_disabled}>Previous</button>'
            f'<span data-httk-serve-table-status role="status" aria-live="polite">{escape(status, quote=False)}</span>'
            f'<button type="button" data-httk-serve-table-next data-token="{next_attr}"{next_disabled}>Next</button>'
            "</nav></section>"
        )
...
def _table_assets() -> tuple[WidgetAsset, ...]:
    ...
    if _TABLE_ASSETS is None:
        ...
        _TABLE_ASSETS = (
            WidgetAsset("serve-table.css", assets.joinpath("serve-table.css").read_bytes(), "text/css"),
            WidgetAsset("serve-table.js", assets.joinpath("serve-table.js").read_bytes(), "text/javascript"),
        )
