...
from httk.serve.web.model.errors import WebError
from httk.serve.web.model.request import HttpRequestContext
from httk.serve.web.widgets.table import (
...
def create_app(*, engine: SiteEngine, debug: bool = False) -> Starlette:
    app = Starlette(
        ...
        routes=[
            Route("/_httk/serve/table/page", _handle_table_page, methods=["GET", "POST"]),
            Route("/_httk/serve/assets/{path:path}", _handle_widget_asset, methods=["GET"]),
            Route("/_httk/serve/{path:path}", _handle_reserved_httk_route, methods=["GET", "POST"]),
...
async def _handle_reserved_httk_route(request: Request) -> Response:
    if request.url.path.startswith("/_httk/serve/assets/"):
