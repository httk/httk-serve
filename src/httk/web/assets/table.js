(() => {
  ...
  const install = (table) => {
    if (table.dataset.httkTableReady === "1") return;
    table.dataset.httkTableReady = "1";
    const previous = table.querySelector("[data-httk-table-previous]");
    const next = table.querySelector("[data-httk-table-next]");
    const status = table.querySelector("[data-httk-table-status]");
    ...
    const requestPage = async (button) => {
      ...
      try {
        ...
        table.dispatchEvent(new CustomEvent("httk:table-updated", {
          bubbles: true,
  ...
  const start = () => document.querySelectorAll("[data-httk-table]").forEach(install);
