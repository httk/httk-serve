import { OptimadeHttpError, OptimadeProtocolError, OptimadeTransport } from "./optimade-table-protocol.mjs";
...
export function installOptimadeTables(root = globalThis.document, options = {}) {
  ...
  return [...root.querySelectorAll("[data-httk-optimade-table]")]
...
export class OptimadeTableController {
  ...
  #commit(page, request, generation) {
    ...
    this.#shell.dispatchEvent(new CustomEventForDocument("httk:optimade-table-updated", {
      bubbles: true,
...
function requiredShell(shell) {
  ...
  const previous = shell.querySelector("[data-httk-optimade-previous]");
  const next = shell.querySelector("[data-httk-optimade-next]");
  const status = shell.querySelector("[data-httk-optimade-status]");
  const pager = shell.querySelector(".httk-optimade-table__pager");
...
function renderRows(tbody, resources, configuration, document) {
  ...
  for (const resource of resources) {
    ...
    for (const column of configuration.columns) {
      ...
      cell.className = `httk-optimade-table__cell httk-optimade-table__cell--${column.align ?? "start"}`;
...
function renderNotice(tbody, columns, message, state, document) {
  tbody.replaceChildren();
  const row = document.createElement("tr");
  row.className = `httk-optimade-table__${state}`;
  const cell = document.createElement("td");
...
function renderProblem(view, columns, message, retry = null) {
  ...
  if (retry) {
    ...
    button.type = "button";
    button.className = "httk-optimade-table__retry";
    button.setAttribute("data-httk-optimade-retry", "1");
...
function removeRetry(view) {
  view.pager?.querySelector("[data-httk-optimade-retry]")?.remove();
}
