...
from httk.web.providers import _validate_site_route
...
def render(
    ...
    *,
    ...
    config_id = escape(f"httk-optimade-table-{context.widget_id}-config", quote=True)
    headers = "".join(
        f'<th scope="col" class="httk-optimade-table__header httk-optimade-table__header--{column["align"]}">'
        f'{escape(column["label"], quote=False)}</th>'
        for column in normalized_columns
    )
    html = (
        f'<link rel="stylesheet" href="{internal_root}/assets/optimade-table.css">'
        f'<script type="module" src="{internal_root}/assets/optimade-table-protocol.mjs"></script>'
        f'<script type="module" src="{internal_root}/assets/optimade-table.mjs"></script>'
        f'<section class="httk-optimade-table" data-httk-optimade-table="1" data-widget-id="{widget_id}" '
        ...
        '<nav class="httk-optimade-table__pager" aria-label="OPTIMADE table pagination">'
        '<button type="button" data-httk-optimade-previous disabled aria-disabled="true">Previous</button>'
        '<span data-httk-optimade-status role="status" aria-live="polite">Loading OPTIMADE results.</span>'
        '<button type="button" data-httk-optimade-next disabled aria-disabled="true">Next</button>'
        "</nav>"
...
def _optimade_table_assets() -> tuple[WidgetAsset, ...]:
    ...
    if _OPTIMADE_TABLE_ASSETS is None:
        ...
        _OPTIMADE_TABLE_ASSETS = (
            WidgetAsset("optimade-table.css", assets.joinpath("optimade-table.css").read_bytes(), "text/css"),
            WidgetAsset("optimade-table.mjs", assets.joinpath("optimade-table.mjs").read_bytes(), "text/javascript"),
            WidgetAsset(
                "optimade-table-protocol.mjs",
                assets.joinpath("optimade-table-protocol.mjs").read_bytes(),
                "text/javascript",
            ),
        )
