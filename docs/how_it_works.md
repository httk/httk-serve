...
the `ServedSchema` from the definitions (validating each column against the
definition), the filter handlers from the columns, and the response-field
extractors from the columns, then loads the records.
