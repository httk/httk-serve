...
from httk.web.publishing.static import publish_site
from httk.web.widgets import MAX_WIDGET_ASSET_BYTES, WidgetAsset, WidgetRenderResult
from httk.web.widgets.assets import WidgetAssetRegistry
...
def test_site_local_declared_asset_is_served_only_after_its_page_renders(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client, TestClient(other_app) as other_client:
        assert client.get("/_httk/assets/site/asset.css").status_code == 404
        assert client.get("/").status_code == 200
        served = client.get("/_httk/assets/site/asset.css")
        assert served.status_code == 200
        assert served.headers["content-type"].startswith("text/css")
        assert served.headers["x-content-type-options"] == "nosniff"
        assert client.post("/_httk/assets/site/asset.css").status_code == 405
        assert other_client.get("/_httk/assets/site/asset.css").status_code == 404
        assert client.get("/_httk/assets/../widgets/asset.py").status_code == 404
        assert client.get("/_httk/assets/%2e%2e/widgets/asset.py").status_code == 404
...
def test_failed_page_asset_registration_is_atomic(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert client.get("/_httk/assets/new.css").status_code == 404
        assert client.get("/valid").status_code == 200
        assert client.get("/_httk/assets/new.css").status_code == 200
...
def test_static_publishing_writes_only_used_assets_and_rejects_static_collision(tmp_path: Path) -> None:
    ...
    assets = tmp_path / "public" / "_httk" / "assets"
    assert (assets / "optimade-table.css").exists()
    assert (assets / "optimade-table.mjs").exists()
    assert (assets / "optimade-table-protocol.mjs").exists()
    assert len([path for path in report.written_files if path.parent == assets]) == 3
    nested = (tmp_path / "public" / "guide" / "index.html").read_text(encoding="utf-8")
    assert 'src="../_httk/assets/optimade-table.mjs"' in nested


@pytest.mark.parametrize(
    ("collision_path", "directory"),
    [
        (Path("_httk/assets/optimade-table.css"), False),
        (Path("_httk/assets/optimade-table.css"), True),
        (Path("_httk/assets"), False),
    ],
)
...
def test_static_publish_ignores_registered_but_unused_asset(tmp_path: Path) -> None:
    ...
    unused = tmp_path / "public" / "_httk" / "assets" / "unused.css"
...
def test_optimade_table_shell_is_safe_and_interactive_in_both_modes(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert 'data-httk-optimade-table="1"' in response.text
        assert 'aria-busy="true"' in response.text
        assert "\\u003c/script\\u003e" in response.text
        assert _optimade_config(response.text)["filter"] == "nsites >= 1"
        assert client.get("/_httk/assets/optimade-table.mjs").status_code == 200
    publish(src, tmp_path / "public", "https://example.test")
    published = (tmp_path / "public" / "index.html").read_text(encoding="utf-8")
    assert "optimade-table.mjs" in published
    assert "data-httk-optimade-next disabled" in published
