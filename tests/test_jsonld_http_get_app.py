"""Coverage for the lightweight live JSON-LD GET application."""
...
def test_document_factories_are_live_and_may_be_async() -> None:
    ...
    app = jsonld_http_get_app(document)
    with TestClient(app) as client:
        first = client.get("/")
        state["generation"] = 2
        second = client.get("/")
...
def test_accept_parsing_supports_quoted_separators_and_rejects_duplicates() -> None:
    ...
    app = jsonld_http_get_app({"@context": {}}, media_type=media_type)
    with TestClient(app) as client:
...
def test_jsonld_app_rejects_noncanonical_paths(path: str) -> None:
    with pytest.raises(ValueError, match="canonical"):
        jsonld_http_get_app({"@context": {}}, path=path)
...
def test_jsonld_app_rejects_non_jsonld_media_type_and_bad_source() -> None:
    with pytest.raises(ValueError, match=r"application/ld\+json"):
        jsonld_http_get_app({"@context": {}}, media_type="application/json")
    with pytest.raises(TypeError, match="mapping"):
        jsonld_http_get_app([{"@context": {}}])  # type: ignore[arg-type]
