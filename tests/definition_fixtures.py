...
httk-optimade is a generic OPTIMADE implementation: entry-type definitions are
...
- ``structures`` and ``widgets`` are synthesized with
  ...
  the real ``structures`` standard, is not a dependency of httk-optimade).
...
builds a :class:`~httk.optimade.schema.served.ServedSchema`.
"""
