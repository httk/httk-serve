...
def test_base_endpoint_is_html() -> None:
    output = process(make_request("/"), StubQueryFunction(), "1.3.0", make_config())
...
def test_entry_endpoint_lists_data() -> None:
    ...
    output = process(make_request("/structures"), query_function, "1.3.0", make_config())
    assert output.json_response is not None
...
def test_entry_endpoint_with_filter_parses_ast() -> None:
    ...
    process(make_request("/structures?filter=nelements=3"), query_function, "1.3.0", make_config())
...
def test_entry_endpoint_with_bad_filter_raises_400() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        process(make_request("/structures?filter=elements HAS"), StubQueryFunction(), "1.3.0", make_config())
...
def test_single_entry_endpoint_builds_id_filter() -> None:
    ...
    output = process(make_request("/structures/abc"), query_function, "1.3.0", make_config())
...
def test_versioned_request() -> None:
    output = process(make_request("/v1/info"), StubQueryFunction(), "1.3.0", make_config())
    assert output.json_response is not None
