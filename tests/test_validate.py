...
def test_base_endpoint() -> None:
    validated = validate_optimade_request(make_request("/"), "1.0.0")
...
def test_fixed_endpoints() -> None:
    for endpoint in ("info", "links", "structures", "calculations", "info/structures"):
        validated = validate_optimade_request(make_request("/" + endpoint), "1.0.0")
        assert validated.endpoint == endpoint


def test_versioned_endpoints() -> None:
    for prefix in ("v1", "v1.0", "v1.0.0"):
        validated = validate_optimade_request(make_request("/" + prefix + "/info"), "1.0.0")
...
def test_unsupported_version() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/v9/info"), "1.0.0")
    assert excinfo.value.response_code == 553


def test_unknown_endpoint() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/nosuch"), "1.0.0")
    assert excinfo.value.response_code == 404


def test_entry_endpoint_with_id() -> None:
    validated = validate_optimade_request(make_request("/structures/abc-123"), "1.0.0")
...
def test_entry_endpoint_with_bad_id() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/structures/\x01bad"), "1.0.0")
    assert excinfo.value.response_code == 400


def test_versions_endpoint_only_unversioned() -> None:
    validated = validate_optimade_request(make_request("/versions"), "1.0.0")
    assert validated.endpoint == "versions"
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/v1/versions"), "1.0.0")
    assert excinfo.value.response_code == 404


def test_page_limit_clamped() -> None:
    validated = validate_optimade_request(make_request("/structures?page_limit=1000"), "1.0.0")
    assert validated.query.page_limit == 50


def test_negative_page_offset_clamped() -> None:
    validated = validate_optimade_request(make_request("/structures?page_offset=-5"), "1.0.0")
    assert validated.query.page_offset == 0


def test_bad_page_limit() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/structures?page_limit=x"), "1.0.0")
    assert excinfo.value.response_code == 400


def test_bad_response_format() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/structures?response_format=xml"), "1.0.0")
    assert excinfo.value.response_code == 400


def test_response_fields_recognized() -> None:
    validated = validate_optimade_request(make_request("/structures?response_fields=elements,nelements"), "1.0.0")
...
def test_response_fields_unknown_spec_field() -> None:
    validated = validate_optimade_request(make_request("/structures?response_fields=species"), "1.0.0")
...
def test_response_fields_unrecognized_raises() -> None:
    with pytest.raises(OptimadeError) as excinfo:
        validate_optimade_request(make_request("/structures?response_fields=nosuchfield"), "1.0.0")
    assert excinfo.value.response_code == 400


def test_response_fields_unknown_prefix_passed_through() -> None:
    validated = validate_optimade_request(make_request("/structures?response_fields=_other_prop"), "1.0.0")
    assert "_other_prop" in validated.unrecognized_response_fields


def test_default_response_fields_used_without_query() -> None:
    validated = validate_optimade_request(make_request("/structures"), "1.0.0")
    assert "elements" in validated.recognized_response_fields
    assert "id" in validated.recognized_response_fields


def test_defaults_not_mutated_by_required_fields() -> None:
    from httk.optimade.schema.httk_entries import default_response_fields

    before = list(default_response_fields["structures"])
    validate_optimade_request(make_request("/structures"), "1.0.0")
    assert default_response_fields["structures"] == before


def test_filter_passed_through() -> None:
    validated = validate_optimade_request(make_request("/structures?filter=nelements=3"), "1.0.0")
...
def test_querystr_used_when_no_query_dict() -> None:
    ...
    validated = validate_optimade_request(request, "1.0.0")
...
def test_determine_optimade_version() -> None:
    assert determine_optimade_version(make_request("/structures")) == "1.0.0"
    assert determine_optimade_version(make_request("/v1/structures")) == "1.0.0"
    assert determine_optimade_version(make_request("/v1.0/structures")) == "1.0.0"
    assert determine_optimade_version(make_request("/")) == "1.0.0"
    with pytest.raises(OptimadeError) as excinfo:
...
def test_determine_optimade_version_two_char_prefix() -> None:
    ...
    assert determine_optimade_version(make_request("/v1/info")) == "1.0.0"
